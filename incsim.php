<?php 
// incsim.php       Norbert Haider, University of Vienna, 2009-2014
// part of MolDB6   last change: 2014-08-07
...
$default_threshold = 0.55; // minimum similraity (according to Tanimoto index)
$max_diffsum = 200;        // cutoff value for molstat-based preselection
...
if ($mol != '') { 
  ...
  if ($use_cmmmsrv == 'y') {
    ...
    if ($ostype == 1) { $chkresult = filterthroughcmd("$safemol", "$CHECKMOL -$options - "); }
    if ($ostype == 2) { $chkresult = filterthroughcmd2("$safemol", "$CHECKMOL -$options - "); }
  }
      ...
      while ($ms_line = mysql_fetch_array($ms_result, MYSQL_ASSOC)) {
        ...
        if ($diffsum <= $max_diffsum) {
          ...
          while ($c_line = mysql_fetch_array($c_result, MYSQL_ASSOC)) {
            ...
            //echo "mol_id: $mol_id  tanimoto: $tanimoto diffsum: $diffsum ($molstat_coefficient)<br>\n";
